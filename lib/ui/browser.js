...
(function() {
  ...
  const utils = {
    ...
    // Fade in animation
    fadeIn: (element, duration = 600, callback) => {
    ...
    animateVolume: (audio, targetVolume, duration = 2000, callback) => {
  ...
  const BrowserUserInterface = function(game, contentElement) {
    this.game = game;
  ...
  BrowserUserInterface.prototype.setSprites = function(data) {
      ...
      if (!this.show_portraits || data === 'none' || data === 'clear') {
          const sprites = ['topLeftSprite', 'topRightSprite', 'bottomLeftSprite', 'bottomRightSprite'];
          sprites.forEach(spriteId => {
              const sprite = document.getElementById(spriteId);
              if (sprite) {
                  Array.from(sprite.children).forEach(child => {
                      utils.fadeOut(child, this.fade_time, () => {
                          sprite.innerHTML = '';
                      });
                  });
              }
  ...
  BrowserUserInterface.prototype.audio = function(audio) {
      ...
      const isLoop = audioData.includes('loop');
      const isQueue = audioData.includes('queue');
      const noFade = audioData.includes('nofade');
      const audioFile = audioData[0];
      const currentAudio = this.currentAudio;
      const fadeTime = this.sound_fade_time;
